...
- **2026-07-21** — Physical Android verification passed for the v1 Claude Code/Codex CLI path. The server emits separate typed chat events, Android renders them by terminal tab with an in-place raw-terminal fallback, and the existing per-tab approval routing remains intact. Extension tests and Android unit/assemble builds pass.
